...
<head>
	...
	<script>
		$(document).ready(function() {
			...
			'<c:if test="${backOrderSelectBox.size() >0 }">'
				//매입처 값 셋팅
				$('#m_mtSaveOrderAcKey').val("${selectOrderKey}").attr("selected", "true");
			'</c:if>'
			...
			$('#m_mtSaveOrderAcKey').change(function(){
				...
				var html = ''; 
				var mtPaymentTotalAmount=0;
				var paymentStatusCdNm = '';
				$.ajax({
		        	url:"/maintenance/contract/detail/paymentPlanInfoWithOrderKey.do",
		            dataType: 'json',
		            type:"post",  
		            ...
		            success:function(data){
		            	//console.log("data.successYN===>"+JSON.stringify(data)+data.successYN);		            	
		            	if("Y" == data.successYN){
		            		
		            		if ( data.paymentPlanList.length > 0 ) {
		            			for(i = 0; i < data.paymentPlanList.length; i++) {
		            				mtPaymentTotalAmount = mtPaymentTotalAmount+data.paymentPlanList[i].paymentTurnAmount;
		            				if(data.paymentPlanList[i].paymentStatusCd == 'W') {
		            					paymentStatusCdNm = '요청대기';
		            				} else if(data.paymentPlanList[i].paymentStatusCd == 'M') {
		            					paymentStatusCdNm = '계산서매핑';
		            				} else if(data.paymentPlanList[i].paymentStatusCd == 'R') {
		            					paymentStatusCdNm = '지급요청';
		            				} else if(data.paymentPlanList[i].paymentStatusCd == 'C') {
		            					paymentStatusCdNm = '지급승인';
		            				} else if(data.paymentPlanList[i].paymentStatusCd == 'E') {
		            					paymentStatusCdNm = '지급완료';
		            				} else if(data.paymentPlanList[i].paymentStatusCd == null) {
		            					paymentStatusCdNm = '지급요청전';
		            				} else {
		            					paymentStatusCdNm =data.paymentPlanList[i].paymentStatusCd;
		            				}
									html += ''
										+ '<tr>'
										+ '	<td>'+(i+1)+'</td>'
										+ '	<td class="textalignC">'+data.paymentPlanList[i].paymentTurn+' 회차</td>'
										+ '	<td class="textalignC">'+makeDateForm(data.paymentPlanList[i].paymentYearMonth)+'</td>'
										+ '	<td class="textalignR">'+addCommas(data.paymentPlanList[i].paymentRate)+'</td>'
										+ '	<td class="textalignR">'+addCommas(data.paymentPlanList[i].paymentTurnAmount)+'</td>'
										+ '	<td>'+addDateMinus(data.paymentPlanList[i].paymentFcDt)+'</td>'
										+ '	<td class="textalignC"><span>'+paymentStatusCdNm+'</span></td>'
										+ '</tr>'		
		            					+ ''
		        				...
		        				$('#mtPaymentTotalAmount').val(addCommas(mtPaymentTotalAmount));
			                } else {
			                	$('.dtl2 tbody').html('');
			                	$('#mtPaymentTotalAmount').val('0');
			                	alert("해당 거래처의 지급 계획정보가 없습니다.");
			                }
		            	} else {
		                	alert("해당 거래처의 지급 계획정보 조회를 실패하였습니다.");
		                }
		...
		*/
		...
		function fn_editMtPanymentPlanBtn(){
			...
			<div class="floatR dpBlock fxd">
				<div class="title">
					<ul>
						...
						<li id="LI_TOPBar_PA" class="on" title="paymentPlanInfo"><label style="cursor: pointer;">지급계획</label></li> 
...
<%-- 					<div class="stitle cg colorBlack">
					...
					<div class="floatC submiddle" style="border-bottom: 2px solid #c4c4c4;">
						<table class="dtl2">
							...
							<tbody>
							<c:set var = "mtPaymentTotalAmount" value="0" />
							<c:forEach var="list" items="${paymentPlanList}" varStatus="status">	
								<c:set var = "mtPaymentTotalAmount" value="${mtPaymentTotalAmount + list.paymentTurnAmount}" />	
								<tr>							
									<td><c:out value="${status.count}"/></td>
									<td class="textalignC"><c:out value="${list.paymentTurn}"/>&nbsp;회차</td>
									<td class="textalignC"><c:out value="${displayUtil.displayDate(list.paymentYearMonth)}"/></td>
									<td class="textalignR"><c:out value="${displayUtil.commaStr(list.paymentRate)}"/>&nbsp;%</td>
									<td class="textalignR"><c:out value="${displayUtil.commaStr(list.paymentTurnAmount)}"/></td>
									<td><c:out value="${displayUtil.displayDate(list.paymentFcDt)}"/></td>
									<td class="textalignC">
										<c:choose>
					...
					<table style="width: 997px; height: 48px;">
						<tr class="bottomtr">
							<td class="textalignR" style="width: 97px">지급금 합계</td>
							<td style="width: 109px">
								<%-- <c:out value="${displayUtil.commaStr(mtSalesTotalAmount)}"/> --%>
								<input type="text" id="mtPaymentTotalAmount" class="pname"  value="<c:out value="${displayUtil.commaStr(mtPaymentTotalAmount)}"/>" readonly />
							</td>
						</tr>
					</table>
					<div class="bottom">
						<div class="floatR">
							<button type="button" value="수정" onclick="fn_editMtPanymentPlanBtn()"><img class="cursorP" src="<c:url value='/images/btn_mod.png'/>" /></button>
						</div>
					</div>
				</div>
			</div>
