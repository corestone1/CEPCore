...
<head>
	...
	<script>
		...
		function goBackOrderDetailInfo() {
			<c:if test='${mtOrderType == "BO"}'> 
			if(confirm("유지보수계약 백계약상세화면으로 이동하시겠습니까?")){
				//window.location.href = "/maintenance/contract/detail/backOrderInfo.do?mtIntegrateKey=${mtIntegrateKey}";
				document.m_mtMovePayment.action = "/maintenance/contract/detail/backOrderInfo.do";
	           	document.m_mtMovePayment.submit();
			}
			</c:if>

			<c:if test='${mtOrderType == "PO"}'> 
			if(confirm("유지보수계약 작업발주 상세화면으로 이동하시겠습니까?")){
				//window.location.href = "/maintenance/work/detail/orderInfo.do?mtIntegrateKey=${mtIntegrateKey}&OrderCtFkKey=${mtWorkKey}";
...
<body>
	<form id="m_mtMovePayment" name="m_mtMovePayment" method="post">
		<input type="hidden" id="m_mtOrderType" name="mtOrderType" value="<c:out value="${mtOrderType}"/>"/><!-- 백계약:BO, 작업발주:PO -->
		<input type="hidden" id="m_mtIntegrateKey" name="mtIntegrateKey" value="<c:out value="${mtIntegrateKey}"/>"/>
		<input type="hidden" id="m_mtWorkKey" name="mtWorkKey" value="<c:out value="${mtWorkKey}"/>"/>	
		<input type="hidden" id="m_orderCtFkKey" name="OrderCtFkKey" value="<c:out value="${mtWorkKey}"/>"/>	
	...
	<div id="listForm" class="listForm">
		...
		<div class="contentsWrap">
			<div class="contents mgauto">
				<div class="floatL">
					<div class="title"><label class="ftw500">유지보수 매입금 지급요청</label></div>
					<div>
							...
							<form>
								<table class="bsc" id="selectBasicTable">
									...
									<tr>
										<td>매입처</td>
										<td>
											<label><c:out value="${basicInfo.orderAcKeyNm}"/></label>
										</td>
									</tr>
									<tr>
										<td>매입처 담당자</td>
										<td>
											<label><c:out value="${basicInfo.acDirectorInfo}"/></label>
										</td>
									</tr>
									<tr>
										<td>결제조건</td>
									...
									<tr>
										<td>발주일자</td>
										<td>
											<label><c:out value="${displayUtil.displayDate(basicInfo.orderDt)}"/></label>
										</td>
									</tr>
									<tr>
										<td>발주금액</td>
										<td>
											<label><c:out value="${displayUtil.commaStr(basicInfo.orderAmount)}"/> 원</label>
										</td>
									</tr>
									<tr>
										<td>지급요청금액</td>
										<td>
											<label><c:out value="${displayUtil.commaStr(basicInfo.callTotalAmount)}"/> 원</label>
										</td>
									</tr>
									<tr>
										<td>기지급액</td>
										<td>
											<label><c:out value="${displayUtil.commaStr(basicInfo.donePaymentAmount)}"/> 원</label>
										</td>
									</tr>
									<tr>
										<td>미지급액</td>
										<td>
											<label><c:out value="${displayUtil.commaStr(basicInfo.yetPaymentAmount)}"/> 원</label>
										</td>
									</tr>
								</table>
							</form>
					...
					<div class="floatL" style="margin-top: 21px">
							<button type="button" title="발주화면으로 이동" class="blueBtnStyle" onclick="goBackOrderDetailInfo()">발주화면 이동</button>
							<%-- <button type="button" title="기본정보수정" value="수정" onclick="goDetailInfo('<c:out value="${basicInfo.mtIntegrateKey}"/>')"><img class="cursorP" src="<c:url value='/images/btn_basic_mod.png'/>" /></button> --%>
						</div>
				</div>
				<div class="floatR dpBlock fxd">
					<%-- <iframe src="/mngMaint/payment/detail/paymentForm.do?pjKey=${basicInfo.mtIntegrateKey}&acKey=${basicInfo.mtAcKey}&billTurnNo=${basicInfo.proceedTurn}&totalTurn=${basicInfo.totalTurn}" width="100%" height="710"></iframe> --%>
					<%-- <iframe id="ifram_payment" src="/mngMaint/payment/detail/prePaymentList.do?mtOrderType=${basicInfo.mtOrderType}&mtIntegrateKey=${mtIntegrateKey}&mtWorkKey=${mtWorkKey}&mtOrderKey=${mtOrderKey}&billAcKey=${basicInfo.orderAcKey}" width="100%" height="710"></iframe> --%>
					<iframe id="ifram_payment" src="${iframUrl}" width="100%" height="710"></iframe>
				</div>
